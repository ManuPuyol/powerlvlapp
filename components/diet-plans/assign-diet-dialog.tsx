...
type AssignDietDialogProps = {
  trigger: React.ReactNode
  dietName: string
  onAssign?: (clientId: string) => void
}

export function AssignDietDialog({ trigger, dietName, onAssign }: AssignDietDialogProps) {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-w-lg">
        ...
        {step === 'select' && (
          <>
            <DialogHeader>
              <p className="font-mono-tag text-muted-foreground">[ ASSIGN DIET ]</p>
              <DialogTitle className="text-2xl tracking-tighter">
                Choose a client
              </DialogTitle>
              <p className="text-sm text-muted-foreground">
                Diet: <span className="text-foreground font-medium">{dietName}</span>
              </p>
            </DialogHeader>
          ...
          <>
            ...
            <div className="space-y-4">
              ...
              <div className="border bg-muted/30 p-4 space-y-2">
                <p className="font-mono-tag text-muted-foreground">DIET</p>
                <p className="font-bold">{dietName}</p>
              </div>

              <p className="text-sm text-muted-foreground">
                The client will be notified and can start the diet immediately.
              </p>
            </div>
